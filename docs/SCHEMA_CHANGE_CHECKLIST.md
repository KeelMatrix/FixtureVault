...
- Boundary regressions must also prove that raw query delimiters establish the complete field before decoding, Azure credential names reject unrelated suffixes, connection-string quoted values remain owned by their parser before generic fallback, and embedded Authorization values stop before ordinary trailing metadata.
