...
- Boundary regressions must also prove that raw query delimiters establish the complete field before decoding, Azure credential names reject unrelated suffixes, connection-string quoted values remain owned only through their closing delimiter before generic fallback, and embedded Authorization values stop before ordinary trailing metadata. A whitespace-separated sibling after every recognized non-secret connection-string indicator must remain independently inspectable.
- JSON representation tests must cover valid nesting below, at, and above the documented 64-container depth. Above-limit valid JSON fails closed with `FV-E014`, `Completed=false`, exit `2`, and no successful-scan telemetry; malformed bracket-prefixed text remains eligible for raw inspection.
