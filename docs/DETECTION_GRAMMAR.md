...
- A structurally valid JSON object, array, or string is decoded exactly once. Every string value, nested value, and array item is then inspected independently. A JSON container is not allowed to suppress a sensitive sibling.
...
- Quoted connection-string values own their quoted contents, including embedded `Pwd=` or `Password=` text. The same text outside that owned value remains eligible for generic detection.
...
Detection and redaction remain separate. FV007 does not mutate fixtures, weaken policy, or disclose matched values. Work is bounded by the scanner's documented read and diagnostic budgets, and strict policy reports a finding with exit code `1`; non-strict policy reports the same finding as a warning with exit code `0`.
