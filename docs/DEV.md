...
The smoke script installs only the built `.nupkg` from an isolated local feed and verifies `--help`, `init`, a clean scan, isolated FV007 cases, exit codes, JSON output, non-disclosure, non-mutation, and the non-strict/`--strict` disposition override. Its corpus covers raw and correctly JSON-serialized connection strings, literal backslashes, JSON escape spellings, quoted/doubled values, field boundaries, generic `api_key`/`client_secret` aliases, decoded JSON property aliases, sibling ordering, `:` ownership after a benign `=` field, and Azure marker/real sibling boundaries; every representative package case is isolated so another source cannot mask the intended result. The CI matrix runs this installed-package corpus on Windows, macOS, Linux x64, and Linux ARM64. For contract changes, use the [Durable Contract Change Checklist](SCHEMA_CHANGE_CHECKLIST.md).
