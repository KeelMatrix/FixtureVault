...
public sealed class FixtureVaultTests
{
    ...
    public static IEnumerable<object[]> GenericCredentialRepresentationAndOwnershipCases()
    {
        ...
        yield return ["generic-colon-comma-harmless-first", $"kind=fixture, token: {canary}", true, canary];
        yield return ["generic-colon-space-harmless-first", $"kind=fixture token: {canary}", true, canary];
        yield return ["generic-colon-prefix", $"metadata token: {canary}", true, canary];
        yield return ["generic-alias-comma-harmless-first", $"kind=fixture, api_key={canary}", true, canary];
        yield return ["generic-adjacent-parenthesis", $"metadata(token: {canary})", true, canary];
        yield return ["generic-adjacent-pipe", $"metadata|api_key={canary}", true, canary];
        yield return ["generic-json-nested-comma-hidden", $"{{\"outer\":[{{\"payload\":\"kind=fixture, token: {canary}\"}}]}}", true, canary];
        yield return ["generic-json-array-prefix-hidden", $"[\"metadata token: {canary}\",\"kind=fixture\"]", true, canary];
        yield return ["generic-marker-comma-benign", "token: [redacted], kind=fixture", false, ""];
        yield return ["generic-benign-comma-marker", "kind=fixture, token: [redacted]", false, ""];
        yield return ["generic-marker-space-benign", "token: [redacted] kind=fixture", false, ""];
        yield return ["generic-benign-space-marker", "kind=fixture token: [redacted]", false, ""];
        yield return ["generic-marker-comma-secret", $"token: [redacted], api_key={canary}", true, canary];
        yield return ["generic-secret-comma-marker", $"api_key={canary}, token: [redacted]", true, canary];
        yield return ["generic-empty-before-unknown-equals", "token= kind=fixture", false, ""];
        yield return ["generic-empty-before-unknown-colon", "token: metadata: fixture", false, ""];
