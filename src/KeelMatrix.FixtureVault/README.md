...
- `FV006` keeps the documented UTF-8 rule for non-Verify text. Verify encoding and newline tolerance are not asserted because FixtureVault cannot prove the repository's canonical `VerifierSettings`. A UTF-16/UTF-32 byte-order mark is decoded so `FV007` still runs, and content that cannot be decoded with any supported encoding is never reported as checked: it appears as a per-file `FV-SKIP-ENCODING` diagnostic and, when sensitive-data detection is enabled, fails the scan closed with `FV-E016` and exit code `2`.
