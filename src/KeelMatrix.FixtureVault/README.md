...
- High-confidence sensitive-data detection recognizes non-empty connection-string `Password` and `Pwd` values case-insensitively in explicit raw and JSON-serialized contexts. Raw fixture text preserves backslash spellings literally; structurally valid JSON string values decode once before raw connection-string parsing, so `\u0022`/`\"` delimiters, escaped whitespace, doubled-quote runs, serialized boundaries, and literal backslashes are classified without double decoding. Empty, whitespace-only, and already-redacted semantic values are ignored without printing matched values.
