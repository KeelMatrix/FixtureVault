...
Prefixed Basic/Bearer headers classify their actual value, so accepted markers in log prefixes remain clean and genuine values remain detectable. Azure names after non-word prefixes remain classifiable without rediscovering text inside a consumed value, and sibling lookahead is local to the current cursor for bounded repeated-field work.
