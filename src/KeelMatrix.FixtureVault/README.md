...
- High-confidence sensitive-data detection recognizes non-empty connection-string `Password` and `Pwd` values case-insensitively, honors doubled-quote escapes and JSON-escaped quoted delimiters, respects serialized string boundaries, and ignores empty, whitespace-only, and already-redacted values without printing matched values.
