...
- Files are opened and read through bounded filesystem checks. Replaced links, special files, file growth/shrinkage, and resource-limit failures return exit code `2`; human-readable paths escape control characters while JSON preserves their actual path values. Findings and skipped diagnostics share a 4,096-record / 1 MiB JSON-field budget; exhaustion returns `FV-E017`, an incomplete scan, exit code `2`, and no successful-scan telemetry.
