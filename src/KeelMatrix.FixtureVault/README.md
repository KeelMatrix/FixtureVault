...
- High-confidence `FV007` detection classifies each structured credential independently: connection-string `Password`/`Pwd`, `AccountKey`/`SharedAccessKey`/`SharedAccessSignature`, API-key headers and URL queries, Basic/Bearer authorization, Cookie/Set-Cookie, and generic assignments. Raw fixture text preserves backslashes; valid JSON string values decode once and query values URL-decode once before their field grammar is parsed. Empty, whitespace-only, and finite accepted redaction markers are ignored, while quote and backslash characters remaining after parsing are data. Matched values are never printed.
