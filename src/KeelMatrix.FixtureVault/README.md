...
- High-confidence sensitive-data detection recognizes non-empty connection-string `Password` and `Pwd` values case-insensitively after decoding supported JSON escapes, including `\u0022` and `\"` quoted delimiters and escaped whitespace. JSON-wrapped doubled-quote runs remain supported, serialized string boundaries are respected, and empty, whitespace-only, and already-redacted semantic values are ignored without printing matched values.
