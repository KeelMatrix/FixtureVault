...
- Corrected FV007 value classification across connection-string credentials, Azure-style key assignments, API-key headers and queries, Basic/Bearer authorization, Cookie/Set-Cookie, and generic assignments. Raw text preserves literal backslashes; valid JSON values and URL query values decode exactly once, field boundaries are parsed before semantic classification, and empty/whitespace/accepted-marker values do not create findings.
