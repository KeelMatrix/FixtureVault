...
- Detects non-empty connection-string `Password`/`Pwd` credentials case-insensitively in raw and JSON-serialized contexts: raw backslash spellings remain literal, while valid JSON string values decode once before raw connection-string parsing. This supports `\u0022`/`\"` delimiters, escaped whitespace, doubled-quote runs, serialized boundaries, and literal backslashes without disclosing secrets.
