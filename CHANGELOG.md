...
- Detects non-empty connection-string `Password`/`Pwd` credentials case-insensitively, including doubled-quote escapes in quoted values and JSON-escaped serialized string boundaries, while ignoring empty, whitespace-only, and already-redacted values without disclosing secrets.
