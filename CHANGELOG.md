...
- Corrected FV007 value classification across connection-string credentials, Azure-style key assignments, API-key headers and queries, Basic/Bearer authorization, Cookie/Set-Cookie, and generic assignments. Generic keys use one case-insensitive alias grammar for `ApiKey`/`api_key`/`api-key`, `ClientSecret`/`client_secret`/`client-secret`, `Password`, `Pwd`, `Secret`, and `Token`, with quoted raw keys and both `=` and `:` operators. Raw text preserves literal backslashes; valid JSON values and URL query values decode exactly once, field boundaries are parsed before semantic classification, clean fields do not suppress sensitive siblings, Azure siblings use semicolon/comma/whitespace boundaries, and empty/whitespace/accepted-marker values do not create findings.
