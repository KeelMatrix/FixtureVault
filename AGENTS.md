...
- Rule IDs `FV001` through `FV008` are stable and must remain documented with their behavior. `FV-SKIP-*` skip codes and `FV-E0xx` error codes are part of the same documented contract. Content-dependent checks must never be skipped without a per-file diagnostic: every counted fixture is either content-inspected as text, or reported with a per-file `FV-SKIP-*` entry. A fixture is only treated as a binary asset that is not text-inspected when its file name has a known binary extension, or when a non-Verify path holds bytes that no supported encoding decodes and that contain NUL characters. A text encoding is proven only from the bytes themselves (UTF-8 with or without a byte-order mark, or UTF-16/UTF-32 declared by a byte-order mark) and never inferred, so a Verify `*.verified.*` or split-mode `*.verified/<file>` baseline whose bytes contain NUL characters without a byte-order mark must be reported as `FV-SKIP-ENCODING` rather than accepted, and content whose text encoding is not proven must fail closed with `FV-E016` whenever sensitive-data detection is enabled.
