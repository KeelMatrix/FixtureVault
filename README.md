...
High-confidence detection recognizes non-empty connection-string `Password` and `Pwd` values case-insensitively in two explicit contexts. Raw connection-string fixture text preserves backslash spellings literally. When a fixture is a structurally valid JSON document, its JSON string values are decoded exactly once and then parsed as raw connection strings; this supports `\u0022`/`\"` quoted delimiters, escaped whitespace, JSON-wrapped doubled-quote runs, and serialized string boundaries without decoding a literal backslash sequence a second time. Empty, whitespace-only, and already-redacted semantic values are not findings. Matched values are never printed in console or JSON output.
...
Sensitive-data detection is separate from redaction: FixtureVault does not rewrite a fixture to clear a finding. Detection uses hardened primitives from `KeelMatrix.Redaction` 0.1.0, with context-sensitive connection-string handling that keeps raw fixture text literal and decodes JSON string values once before raw parsing; the matched value is never retained in a report or diagnostic.
