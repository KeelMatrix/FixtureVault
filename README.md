...
Authorization matches embedded in log prefixes are classified by their actual Basic/Bearer value, not by the redaction transform's changed header text. Azure prefix ownership and sibling lookahead remain bounded, so supported values are found without rescanning consumed values or quadratic suffix searches.
