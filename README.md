...
High-confidence detection recognizes non-empty connection-string `Password` and `Pwd` values case-insensitively. Quoted values honor doubled-quote escapes and JSON-escaped quoted delimiters, and unquoted values stop at serialized string boundaries, so JSON-wrapped empty, whitespace-only, and already-redacted values are not findings. Matched values are never printed in console or JSON output.
