...
High-confidence detection recognizes non-empty connection-string `Password` and `Pwd` values case-insensitively after decoding supported JSON escapes, including `\u0022` and `\"` quoted delimiters and escaped whitespace. JSON-wrapped doubled-quote runs remain supported, and unquoted values stop at serialized string boundaries, so JSON-wrapped empty, whitespace-only, and already-redacted semantic values are not findings. Matched values are never printed in console or JSON output.
