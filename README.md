...
- [FV007 detection grammar](https://github.com/KeelMatrix/FixtureVault/blob/main/docs/DETECTION_GRAMMAR.md)
...
High-confidence `FV007` independently detects structured credential values and never prints matched values. Structurally valid JSON is decoded once through a maximum depth of 64 containers; a valid deeper representation fails closed with `FV-E014`, exit code `2`, and no successful-scan telemetry. Malformed or non-JSON bracket-prefixed text remains eligible for raw inspection. See the [canonical FV007 detection grammar](https://github.com/KeelMatrix/FixtureVault/blob/main/docs/DETECTION_GRAMMAR.md) for the complete ownership, depth, and boundary contract.
...
- **Sensitive-data detection:** `FV-E014` means an enabled sensitive-data detector could not complete, including a valid JSON representation deeper than the supported 64-container limit. The scan exits `2`, reports an incomplete result, and does not activate telemetry. Malformed JSON is still eligible for raw-text inspection.
