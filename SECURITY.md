...
- The complete FV007 ownership and boundary contract is maintained in the [canonical detection grammar](https://github.com/KeelMatrix/FixtureVault/blob/main/docs/DETECTION_GRAMMAR.md).
...
- `FV007` independently detects high-confidence structured credentials and never discloses matched values. Connection-string masking is limited to syntactically owned value spans, so unrelated clean context cannot suppress a finding while quoted non-secret values remain owned. See the [canonical FV007 detection grammar](https://github.com/KeelMatrix/FixtureVault/blob/main/docs/DETECTION_GRAMMAR.md) for the complete security boundary contract.
