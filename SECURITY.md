...
- Embedded Basic/Bearer headers classify only the complete credential value, excluding ordinary trailing log metadata. Query fields end at raw ampersand or hash separators before one URL decode; connection-string parsed value spans are not rescanned as generic assignments; and Azure prefix ownership ends at the complete recognized field name.
...
- Embedded Basic/Bearer headers are classified by their actual value, so accepted markers in harmless prefixes remain clean while genuine values remain findings. Azure prefix ownership is bounded to the recognized field name, and sibling lookahead stays local to the current cursor rather than rescanning the remaining suffix.
