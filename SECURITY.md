...
- Connection-string `Password`/`Pwd` detection decodes supported JSON escapes, including `\u0022` and `\"` quoted delimiters and escaped whitespace. It honors JSON-wrapped doubled-quote runs and serialized string boundaries, ignores empty/whitespace-only/already-redacted semantic values, and never emits matched values in console or JSON diagnostics.
