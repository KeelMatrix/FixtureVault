...
- Connection-string `Password`/`Pwd` detection is context-sensitive: raw connection-string text preserves backslash escape spellings literally, while structurally valid JSON string values are decoded exactly once before raw connection-string parsing. This covers `\u0022`/`\"` delimiters, escaped whitespace, JSON-wrapped doubled-quote runs, serialized boundaries, and double-encoded literal backslashes without applying JSON decoding to raw values. Empty/whitespace-only/already-redacted semantic values are ignored, and matched values never appear in console or JSON diagnostics.
