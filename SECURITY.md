...
- `FV007` classifies structured credential values independently for connection-string `Password`/`Pwd`, `AccountKey`/`SharedAccessKey`/`SharedAccessSignature`, API-key headers and URL queries, Basic/Bearer authorization, Cookie/Set-Cookie, and generic assignments. Raw text preserves literal backslashes; valid JSON string values decode once and query values URL-decode once before field parsing. Empty, whitespace-only, and finite accepted redaction markers are clean, while parsed quote/backslash data remains sensitive. Field boundaries, quoted delimiters, repeated keys, and embedded separators are handled without disclosing matched values in console or JSON diagnostics.
